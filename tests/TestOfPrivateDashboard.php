...
class TestOfPrivateDashboard extends ThinkTankWebTestCase {
    ...
    public function setUp() {
        ...
        $q = "INSERT INTO tt_users (user_id, user_name, full_name, avatar) VALUES (12, 'jack', 'Jack Dorsey',
        'avatar.jpg');";
        $this->db->exec($q);
        ...
        $q = "INSERT INTO tt_users (user_id, user_name, full_name, avatar, is_protected, follower_count) VALUES (17,
        'thinktankapp', 'ThinkTankers', 'avatar.jpg', 0, 10);";
        $this->db->exec($q);

        $q = "INSERT INTO tt_users (user_id, user_name, full_name, avatar, is_protected, follower_count) VALUES (18,
        'shutterbug', 'Shutter Bug', 'avatar.jpg', 0, 10);";
        $this->db->exec($q);

        $q = "INSERT INTO tt_users (user_id, user_name, full_name, avatar, is_protected, follower_count) VALUES (19,
        'linkbaiter', 'Link Baiter', 'avatar.jpg', 0, 10);";
        $this->db->exec($q);

        $q = "INSERT INTO tt_user_errors (user_id, error_code, error_text, error_issued_to_user_id) VALUES (15, 404,
        'User not found', 13);";
        $this->db->exec($q);
        ...
        while ($counter < 40) {
            ...
            $q = "INSERT INTO tt_links (url, expanded_url, title, clicks, post_id, is_image) VALUES
        ...
        while ($counter < 10) {
            ...
            'thinktankapp', 'thinktankapp', 'avatar.jpg', 'This is test post $counter', 'web', 
            '2006-03-01 00:$pseudo_minute:00', 0, 0);";
            $this->db->exec($q);
            $counter++;
        }
    ...
    public function testUserPage() {
        ...
        $this->get($this->url.'/user/index.php?i=thinktankapp&u=ev&n=twitter');
        $this->assertTitle('User Details: ev | ThinkTank');
        $this->assertText('Logged in as: me@example.com');
        $this->assertText('ev');
