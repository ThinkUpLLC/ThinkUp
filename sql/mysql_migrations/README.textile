...
h2. Version Migrations

When a new version of ThinkTank gets tagged, all the database migration scripts associated with it get rolled up into a single script that ends with <code>.sql.migration</code>. For example, version 0.001 was tagged v0.001 on February 27th. Therefore, the migration associated with it is named <code>2010-02-27_v0.001.sql.migration</code>.

h2. Migrations Since the Last Version Release
...
<code>mysql -u yourusername -p thinktank < /path/to/thinktank/sql/mysql_migrations/YYY_MM_DD_script-name.sql</code>
