...
The file to query for API results is located in <thinkup>/webapp/api/v1/post.php. There are a number of API call 
...
able to query for protected posts using the API. The API will only return posts that are publicly visible on Twitter.

How do I use it?
----------------

Using it is quite simple. Let's say you wanted to make an API call of type "user_posts" for the user "samwhoo", your
request would look like this:

`http://example.com/your_thinkup_install/webapp/api/v1/post.php?type=user_posts&username=samwhoo`

And that will output the latest 20 posts made by samwhoo (as the default number of posts to return is 20).
...
This `Anatomy of a Tweet <http://www.scribd.com/doc/30146338/map-of-a-tweet>`_ PDF is what was followed when determining
the fields to use and what data to put in them. It's a very good start for anyone wishing to learn the Twitter or 
ThinkUp API.
