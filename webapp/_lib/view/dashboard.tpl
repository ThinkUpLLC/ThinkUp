...
<div class="container_24 clearfix">
  ...
  <div class="grid_20 alpha thinkup-canvas round-all">
    <div class="prepend_20 append_20 prefix_1 suffix_1">
        ...
        {else}
          ...
          <div class="clearfix prepend">
              <div class="grid_9 alpha">
                  ...
                  {else} 
                      ...
                      <img src="http://chart.apis.google.com/chart?chs=350x200&chxt=x,y&chxl=0:|{foreach from=$follower_count_history_by_day.history key=tid item=t name=foo}{$t.date}|{/foreach}1:|{foreach from=$follower_count_history_by_day.y_axis key=tid item=t name=foo}{$t|number_format}{if !$smarty.foreach.foo.last}|{/if}{/foreach}&cht=ls&chco=0077CC&chd=t:{foreach from=$follower_count_history_by_day.percentages key=tid item=t name=foo}{$t}{if !$smarty.foreach.foo.last},{/if}{/foreach}&chm=B,76A4FB,0,0,0&chg=33">
              ...
              <div class="grid_9 omega">
                  ...
                  {else} 
                      ...
                      <img src="http://chart.apis.google.com/chart?chs=350x200&chxt=x,y&chxl=0:|{foreach from=$follower_count_history_by_week.history key=tid item=t name=foo}{$t.date}|{/foreach}1:|{foreach from=$follower_count_history_by_week.y_axis key=tid item=t name=foo}{$t|number_format}{if !$smarty.foreach.foo.last}|{/if}{/foreach}&cht=ls&chco=0077CC&chd=t:{foreach from=$follower_count_history_by_week.percentages key=tid item=t name=foo}{$t}{if !$smarty.foreach.foo.last},{/if}{/foreach}&chm=B,76A4FB,0,0,0&chg=33">
          ...
          <div class="clearfix">
              ...
              <div class="grid_6 alpha">
                  ...
                  <img src="http://chart.apis.google.com/chart?chxt=x,y&cht=bhg&chd=t:{$instance->percentage_replies|round},{$instance->percentage_links|round}&chco=76A4FB&chls=2.0&chs=230x150&chxl=1:|Broadcaster|Conversationalist|0:||20%||60%||100%|&chbh=50" height="150" width="230" />
              ...
              <div class="grid_12 omega">
                  ...
                  <img src="http://chart.apis.google.com/chart?chs=470x200&cht=p&chd=t:{foreach from=$all_time_clients_usage key=name item=num_posts name=foo}{math equation="round(x/y*100,2)" x=$num_posts y=$all_time_clients_usage|@array_sum}{if !$smarty.foreach.foo.last},{/if}{/foreach}&chl={foreach from=$all_time_clients_usage key=name item=num_posts name=foo}{$name}+({$num_posts}){if !$smarty.foreach.foo.last}|{/if}{/foreach}&chco=76A4FB">
              </div>
          </div>
          {/if}
