...
<ul>
<li><a href="beta.php?u={$instance->network_username|urlencode}&n={$instance->network}">Main Dashboard</a></li>
<li>Posts<ul>
{foreach from=$sidebar_menu_posts key=mkey item=mi name=tabloop}
   <li><a href="beta.php?v={$mkey}&u={$instance->network_username|urlencode}&n={$instance->network}">{$mi}</a></li>
{/foreach}</ul></li>
<li>Replies<ul>
