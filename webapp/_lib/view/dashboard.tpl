    ...
    {else}
        ...
        {if $instance->network eq 'twitter' }
            ...
            <div class="grid_10 alpha">
                ...
                <img src="http://chart.apis.google.com/chart?chxt=x,y&cht=bhg&chd=t:{$instance->percentage_replies|round},{$instance->percentage_links|round}&chco=76A4FB&chls=2.0&chs=400x150&chxl=1:|Broadcaster|Conversationalist|0:||20%||60%||100%|&chbh=50" />
                </div>
            </div>
            <div class="grid_10 omega">
              Client Usage <span class="detail">(all posts)</span><br />
              <img src="http://chart.apis.google.com/chart?cht=p&chd=t:{foreach from=$all_time_clients_usage key=name item=num_posts name=foo}{if $num_posts>0}{math equation="round(x/y*100,2)" x=$num_posts y=$all_time_clients_usage|@array_sum}{/if}{if !$smarty.foreach.foo.last},{/if}{/foreach}&chs=400x200&chl={foreach from=$all_time_clients_usage key=name item=num_posts name=foo}{$name}+({$num_posts}){if !$smarty.foreach.foo.last}|{/if}{/foreach}&chco=76A4FB">
            </div>
        {/if}
