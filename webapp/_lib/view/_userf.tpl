...
<div class="individual-tweet clearfix{if $t.is_protected} private{/if}">
  <div class="grid_1 alpha">    <a href="{$site_root_path}user/?u={$f.user_name}&n={$f.network}"><img src="{$f.avatar}" class="avatar"/><img src="{$site_root_path}plugins/{$f.network|get_plugin_path}/assets/img/favicon.ico" class="service-icon"/></a>
  </div>
  <div class="grid_3 small right">
    <a href="{$site_root_path}user/?u={$f.user_name}&n={$f.network}">{$f.user_name}</a>
  </div>
  <div class="grid_16 omega">
    ...
    {if $f.avg_tweets_per_day}
      ...
      {/if}
    {else}
      <div class="small gray">Joined {$f.joined|relative_datetime} ago.</div>
      {if $f.follower_count}
        <div class="small gray">{$f.follower_count} follower(s).</div>
      {/if}
    {/if}
  </div>
</div>
