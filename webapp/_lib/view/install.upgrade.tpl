...
<div class="container_24 thinkup-canvas round-all">
  ...
  <div class="clearfix prepend_20">
    ...
    <!-- no upgrade needed -->
     <div class="ui-state-success ui-corner-all" style="margin: 20px 0px; padding: 0.5em 0.7em;">
         <p>
           ...
           Your database is up to date. <a href="{$site_root_path}">Continue using {$app_title}</a>, or <a href="backup.php">back up your database.</a>
                {if $version_updated}
                <p>Your application database version has been updated to reflect the latest installed version of {$app_title}.</p>
                {/if}
        </p>
     </div> 
    {else}
    <div id="info-parent" class="ui-state-highlight ui-corner-all" style="margin: 0px 50px 0px 50px; padding: 0.5em 0.7em;">
        ...
        Your {$app_title} installation needs {$migrations|@count} database update{if $migrations|@count gt 1}s{/if}. {if $user_is_admin}Before you proceed, 
        <a href="{$site_root_path}install/backup.php">back up your current {$app_title} database</a>.{else}<br />If you haven't already, <a href="http://thinkupapp.com/docs/install/backup.html">back up your current installation's data first</a>.{/if}
        </p>
        </div>
    ...
    <br /><br />
        ...
        value="Update {$app_title}'s Database" type="submit" style="font-size:24px;line-height:2.2em;">
        </form>
        </div>
     </div>
