...
<div class="panel panel-default insight insight-default insight-{$i->slug|replace:'_':'-'}
  ...
  (isset($i->related_data.hero_image) and $i->emphasis > '1') | $i->slug eq 'weekly_graph'}insight-wide{/if} {if $insight->slug|strpos:'eoy_'===0}insight-eoy insight-wide{/if}" id="insight-{$i->id}">
