...
{foreach from=$insights key=tid item=i name=insights}
  ...
  {capture name=permalink assign="permalink"}{$thinkup_application_url}?u={$i->instance->network_username|replace_accented_characters}&n={$i->instance->network}&d={$i->date|date_format:'%Y-%m-%d'}&s={$i->slug}{/capture}
