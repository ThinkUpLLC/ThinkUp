...
class FavoritePostMySQLDAO extends PostMySQLDAO implements FavoritePostDAO  {
  ...
  /**
   * Inserts the given post record (if it does not already exist), then creates a row in the favorites 'join' table to 
   * store information about the 'favorited' relationship. $vals holds the parsed post information.
   * @param array $vals
  ...
  public function addFavorite($favoriter_id, $vals) {
    ...
    $q = "
        INSERT IGNORE INTO #prefix#favorites
  ...
  public function unFavorite($tid, $uid, $network = 'twitter') {
    $q = "
      DELETE FROM #prefix#favorites where status_id = :tid AND fav_of_user_id = :uid AND network = :network";
    $vars = array(
        ':tid' => $tid,
        ':uid' => $uid,
        ':network' => $network,
   ...
   */
  private function getAllFPostsByUserID($owner_id, $network, $count, $order_by="pub_date", 
    $direction="DESC", $ubound = null, $page=1, $iterator = false) {
      ...
      $q = "select l.*, p.*, pub_date - interval #gmt_offset# hour as adj_pub_date from (#prefix#posts p LEFT JOIN #prefix#favorites f on f.status_id = p.post_id) LEFT JOIN #prefix#links l on l.post_id = p.post_id where f.fav_of_user_id = :owner_id AND p.network=:network ";
  ...
  $order_by="pub_date", $in_last_x_days = 0, $iterator = false) {
      ...
      $q = "select l.*, p.*, pub_date - interval #gmt_offset# hour as adj_pub_date from ((#prefix#posts p LEFT JOIN #prefix#favorites f on f.status_id = p.post_id) LEFT JOIN #prefix#links l on l.post_id = p.post_id) LEFT JOIN #prefix#users u on u.user_id = f.fav_of_user_id where u.user_name = :author_username AND p.network=:network ";
