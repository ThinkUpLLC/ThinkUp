 ...
 * and email that Token to the admin users. This will allow users to upgrade the app without needing to authenticate,
...
class UpgradeController extends ThinkUpAuthController {
     ...
     * @return bool
     */
    public static function isUpgrading($isadmin = false, $classname) {
        $config = Config::getInstance();
        $status = false;
        $db_version = UpgradeController::getCurrentDBVersion(true);
