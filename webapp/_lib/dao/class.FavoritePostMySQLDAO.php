...
class FavoritePostMySQLDAO extends PostMySQLDAO implements FavoritePostDAO {
	public function addFavorite($favoriter_id, array $vals, $entities = null, $user_array = null) {
		...
		if (! $post) {
			...
			if (! $added_post) {
				throw new Exception ( "Error: favorited post ID " . $vals ['post_id'] . " is not in storage and could not be inserted." );
			}
		}
	...
	public function getAllFavoritePosts($owner_id, $network, $count, $page = 1, $is_public = false) {
		return $this->getAllFavoritePostsByUserID ( $owner_id, $network, $count, "pub_date", "DESC", null, $page, false, $is_public );
	}
	 ...
	 */
	private function getAllFavoritePostsByUserID($owner_id, $network, $count, $order_by = "pub_date", $direction = "DESC", $ubound = null, $page = 1, $iterator = false, $is_public = false) {
	 ...
	 * @return array Posts with link object set or PostIterator
	 */
	private function getAllFavoritePostsByUsernameOrderedBy($author_username, $network = "twitter", $count = 0, $order_by = "pub_date", $in_last_x_days = 0, $iterator = false) {
		if (! in_array ( $order_by, $this->REQUIRED_FIELDS ) && ! in_array ( $order_by, $this->OPTIONAL_FIELDS )) {
			$order_by = "pub_date";
		}
		$vars = array (
	 ...
	 * @param int $page        	
	 * @param bool $iterator
	 *        	Whether or not to return an iterator
	 * @return s array Post objects
	 */
	...
	// public function getFavoritedToday($author_user_id, $network) {
	// echo "getFavoritedToday";
	...
	// $q = "SELECT #prefix#users.gender, COUNT(*) as count_gender FROM #prefix#favorites, #prefix#users ";
	// $q .= "WHERE #prefix#favorites.post_id = ".$post->post_id." ";
	// $q.= "AND #prefix#favorites.fav_of_user_id = #prefix#users.user_id ";
	// $q .= "GROUP BY #prefix#users.gender";
	
	// if ($this->profiler_enabled) { Profiler::setDAOMethod(__METHOD__); }
	
	// $ps_likes = $this->execute($q);
	// $rows_likes = $this->getDataRowsAsArrays($ps_likes);
	
	// foreach ($rows_likes as $row_likes) {
	// if ($row_likes['gender'] == "female")
	// $post->female_likes_count = $row_likes['count_gender'];
	// if ($row_likes['gender'] == "male")
	// $post->male_likes_count = $row_likes['count_gender'];
	// }
	
	// $q = "SELECT #prefix#users.gender, COUNT(*) as count_gender FROM #prefix#posts, #prefix#users ";
	// $q .= "WHERE #prefix#posts.in_reply_to_post_id = ".$post->post_id." ";
	// $q .= "AND #prefix#posts.author_user_id = #prefix#users.user_id ";
	// $q .= "GROUP BY #prefix#users.gender";
	
	// if ($this->profiler_enabled) { Profiler::setDAOMethod(__METHOD__); }
	
	// $ps_comm = $this->execute($q);
	// $rows_comm = $this->getDataRowsAsArrays($ps_comm);
	
	// foreach ($rows_comm as $row_comm) {
	// if ($row_comm['gender'] == "female")
	// $post->female_comm_count = $row_comm['count_gender'];
	// if ($row_comm['gender'] == "male")
	// $post->male_comm_count = $row_comm['count_gender'];
	// }
	// $posts[] = $post;
	// }
