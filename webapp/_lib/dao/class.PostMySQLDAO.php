...
class PostMySQLDAO extends PDODAO implements PostDAO {
	...
	public function getRepliesToPost($post_id, $network, $order_by = 'default', $unit = 'km', $is_public = false, $count = 350, $page = 1) {
		...
		if ($all_post_rows) {
			...
			foreach ( $all_post_rows as $post_row ) {
				if ($post_row ['is_geo_encoded'] == 1) {
					...
					if ($unit == 'mi') {
						$post_row ['reply_retweet_distance'] = $this->calculateDistanceInMiles ( $post_row ['reply_retweet_distance'] );
					}
				}
	...
	public function getRepliesToPostInRange($post_id, $network, $from, $until, $order_by = 'default', $unit = 'km', $is_public = false, $count = 350, $page = 1) {
		$start_on_record = ($page - 1) * $count;
	...
	public function getRepliesToPostIterator($post_id, $network, $order_by = 'default', $unit = 'km', $is_public = false, $count = 350, $page = 1) {
		$start_on_record = ($page - 1) * $count;
	...
	public function getRetweetsOfPost($post_id, $network = 'twitter', $order_by = 'default', $unit = 'km', $is_public = false, $count = null, $page = 1) {
		$q = "SELECT u.*, p.*, (CASE p.is_geo_encoded WHEN 0 THEN 9 ELSE p.is_geo_encoded END) AS geo_status, ";
		$q .= "pub_date + interval #gmt_offset# hour as adj_pub_date ";
		$q .= "FROM #prefix#posts p ";
	...
	public function getRelatedPostsArray($post_id, $network = 'twitter', $is_public = false, $count = 350, $page = 1, $geo_encoded_only = true, $include_original_post = true) {
		$start_on_record = ($page - 1) * $count;
		$q = "SELECT * FROM (
	...
	public function getRelatedPosts($post_id, $network = 'twitter', $is_public = false, $count = 350, $page = 1, $geo_encoded_only = true, $include_original_post = true) {
		$all_rows = $this->getRelatedPostsArray ( $post_id, $network, $is_public, $count, $page, $geo_encoded_only, $include_original_post );
		$responses = array ();
		$location = array ();
		foreach ( $all_rows as $row ) {
			if ($row ['is_geo_encoded'] == 1) {
	...
	/**
	 * Update the 'in_retweet_of_post_id' field for an existing post.
	 ...
	 * @param str $network        	
	 * @param str $fieldname
	 *        	either "reply" , "old_retweet" (old-style retweets), "native_retweet" or "favlike"
	...
	public function addPost($vals) {
		...
		if (isset ( $vals ['retweeted_post'] )) {
			...
			$this->addPostAndAssociatedInfo ( $retweeted_post_data, $retweeted_post_entities, $retweeted_post_user_array );
		}
		...
		if ($this->hasAllRequiredFields ( $vals )) {
			...
			if (isset ( $vals ['in_reply_to_post_id'] ) && $vals ['in_reply_to_post_id'] != '') {
				...
				if (isset ( $replied_to_post )) {
					...
					if ($follow_dao->followExists ( $vals ['author_user_id'], $replied_to_post->author_user_id, $replied_to_post->network )) {
			...
			if (! $res && ! isset ( $retweeted_post_data ) && isset ( $vals ['in_retweet_of_post_id'] ) && $this->updateInRetweetOfPostID ( $vals ['post_id'], $vals ['in_retweet_of_post_id'], $vals ['network'] )) {
				$this->incrementRepostCountCache ( $vals ['in_retweet_of_post_id'], $vals ['network'] );
				$status_message = "Repost of " . $vals ['in_retweet_of_post_id'] . " by " . $vals ["author_username"] . " ID: " . $vals ["post_id"] . "; updating old-style retweet cache count";
			...
			if ($res) {
				...
				if (isset ( $retweeted_post )) {
					if (! isset ( $retweeted_post_data )) { // if not a native retweet
						...
						$status_message = "Retweet of " . $vals ['in_retweet_of_post_id'] . " by " . $vals ["author_username"] . " ID: " . $vals ["post_id"];
						if ($count > 0) {
	...
	public function getAllPosts($author_id, $network, $count, $page = 1, $include_replies = true, $order_by = 'pub_date', $direction = 'DESC', $is_public = false) {
		return $this->getAllPostsByUserID ( $author_id, $network, $count, $order_by, $direction, $include_replies, $page, false, $is_public );
	}
	public function getAllPostsIterator($author_id, $network, $count, $include_replies = true, $order_by = 'pub_date', $direction = 'DESC', $is_public = false) {
		return $this->getAllPostsByUserID ( $author_id, $network, $count, $order_by, $direction, $include_replies, 1, $iterator = true, $is_public );
	}
	...
	public function getPostsByFriends($user_id, $network, $count = 15, $page = 1, $is_public = false, $iterator = false) {
		$start_on_record = ($page - 1) * $count;
		if ($is_public) {
	...
	public function getAllQuestionPosts($author_id, $network, $count, $page = 1, $order_by = 'pub_date', $direction = 'DESC', $is_public = false) {
		$start_on_record = ($page - 1) * $count;
	...
	public function getAllQuestionPostsInRange($author_id, $network, $count, $from, $until, $page = 1, $order_by = 'pub_date', $direction = 'DESC', $is_public = false) {
		$order_by = $this->sanitizeOrderBy ( $order_by );
	 ...
	 * @param str $direction
	 *        	either "DESC" or "ASC
	 * @param bool $include_replies
	 *        	If true, return posts with in_reply_to_post_id set, if not don't
	 * @param int $page
	 *        	Page number, defaults to 1
	 * @param bool $iterator
	 *        	Specify whether or not you want a post iterator returned. Default to
	 *        	false.
	 * @param bool $is_public
	 *        	Whether or not these results are going to be displayed publicly. Defaults to false.
	 * @return array Posts with link object set
	 */
	private function getAllPostsByUserID($author_id, $network, $count, $order_by = "pub_date", $direction = "DESC", $include_replies = true, $page = 1, $iterator = false, $is_public = false) {
		$direction = $direction == "DESC" ? "DESC" : "ASC";
		$start_on_record = ($page - 1) * $count;
		$order_by = $this->sanitizeOrderBy ( $order_by );
		...
		if ($order_by == 'retweets') {
			$q = "SELECT p.*, (p.retweet_count_cache + p.old_retweet_count_cache) as retweets, " . "pub_date + interval #gmt_offset# hour as adj_pub_date ";
		} else {
			$q = "SELECT p.*, pub_date + interval #gmt_offset# hour as adj_pub_date ";
		}
		$q .= "FROM #prefix#posts p ";
	...
	public function getPostsByUserInRange($author_id, $network, $from, $until, $order_by = "pub_date", $direction = "DESC", $iterator = false, $is_public = false) {
		$direction = $direction == "DESC" ? "DESC" : "ASC";
	 ...
	 *        	False if no limit (ie, return all rows)
	 * @param str $order_by
	 *        	field name Default "pub_date"
	 * @return array Posts with link object set
	 */
	public function getAllPostsByUsernameOrderedBy($author_username, $network = "twitter", $count = 0, $order_by = "pub_date", $in_last_x_days = 0, $iterator = false, $is_public = false, $since = null) {
		$order_by = $this->sanitizeOrderBy ( $order_by );
		$vars = array (
		...
		// which we can then sort on.
		if ($order_by == 'retweets') {
			$q = "SELECT p.*, GREATEST((p.retweet_count_cache + p.old_retweet_count_cache), retweet_count_api) " . "as retweets, pub_date + interval #gmt_offset# hour as adj_pub_date ";
		} else {
			$q = "SELECT p.*, pub_date + interval #gmt_offset# hour as adj_pub_date ";
		}
		$q .= "FROM #prefix#posts p ";
	...
	public function getMostRepliedToPostsInLastWeek($username, $network, $count, $is_public = false) {
		return $this->getAllPostsByUsernameOrderedBy ( $username, $network, $count, 'reply_count_cache', 7, $iterator = false, $is_public );
	}
	public function getMostFavedPostsInLastWeek($username, $network, $count, $is_public = false) {
		return $this->getAllPostsByUsernameOrderedBy ( $username, $network, $count, 'favlike_count_cache', 7, $iterator = false, $is_public );
	}
	public function getMostRetweetedPostsInLastWeek($username, $network, $count, $is_public = false) {
		return $this->getAllPostsByUsernameOrderedBy ( $username, $network, $count, 'retweets', 7, $iterator = false, $is_public );
	}
	...
	public function getAllMentionsIterator($author_username, $count, $network = "twitter", $page = 1, $public = false, $include_rts = true, $order_by = 'pub_date', $direction = 'DESC') {
		return $this->getMentions ( $author_username, $count, $network, $iterator = true, $page, $public, $include_rts, $order_by, $direction );
	}
	public function getAllMentions($author_username, $count, $network = "twitter", $page = 1, $public = false, $include_rts = true, $order_by = 'pub_date', $direction = 'DESC') {
		return $this->getMentions ( $author_username, $count, $network, $iterator = false, $page, $public, $include_rts, $order_by, $direction );
	}
	private function getMentions($author_username, $count, $network, $iterator, $page = 1, $public = false, $include_rts = true, $order_by = 'pub_date', $direction = 'DESC') {
		$start_on_record = ($page - 1) * $count;
	...
	public function getAllMentionsInRange($author_username, $count, $network = "twitter", $from, $until, $page = 1, $public = false, $include_rts = true, $order_by = 'pub_date', $direction = 'DESC') {
		return $this->getMentionsInRange ( $author_username, $count, $network, $from, $until, $iterator = false, $page, $public, $include_rts, $order_by, $direction );
	}
	private function getMentionsInRange($author_username, $count, $network, $from, $until, $iterator, $page = 1, $public = false, $include_rts = true, $order_by = 'pub_date', $direction = 'DESC') {
		$start_on_record = ($page - 1) * $count;
	...
	public function getAllReplies($user_id, $network, $count, $page = 1, $order_by = 'pub_date', $direction = 'DESC', $is_public = false) {
		$start_on_record = ($page - 1) * $count;
	...
	public function getAllRepliesInRange($user_id, $network, $count, $from, $until, $page = 1, $order_by = 'pub_date', $direction = 'DESC', $is_public = false) {
		$start_on_record = ($page - 1) * $count;
	...
	public function getMostRepliedToPosts($user_id, $network, $count, $page = 1, $is_public = false) {
		return $this->getAllPostsByUserID ( $user_id, $network, $count, "reply_count_cache", "DESC", true, $page, $iterator = false, $is_public );
	}
	public function getMostFavedPosts($user_id, $network, $count, $page = 1, $is_public = false) {
		return $this->getAllPostsByUserID ( $user_id, $network, $count, "favlike_count_cache", "DESC", true, $page, $iterator = false, $is_public );
	}
	public function getMostRetweetedPosts($user_id, $network, $count, $page = 1, $is_public = false) {
		return $this->getAllPostsByUserID ( $user_id, $network, $count, "retweets", "DESC", true, $page, $iterator = false, $is_public );
	}
	...
	public function setGeoencodedPost($post_id, $network, $is_geo_encoded = 0, $location = null, $geodata = null, $distance = 0) {
	 ...
	 *
	 * @param int $location
	 *        	Location as stored in the database
	 ...
	 * To clean up the clients names, we remove the HTML link tag.
	 *
	 * @param array $rows
	 *        	obtained from the database (as array); columns should be 'num_posts' and 'source'
	...
	public function countCheckinsToPlaceTypes($author_id, $network) {
		...
		foreach ( $all_rows as $row ) {
			/*
			 * Data needs to be in format {c:[{v: 'Title'}, {v: Value of data for this title}]} e.g. [{c:[{v: 'Park'}, {v: 5}]}
			 */
			$resultset [] = array (
					'c' => array (
							array (
		...
		$metadata = array (
				...
				array (
						'type' => 'number',
						'label' => 'Number of Checkins to this place type' 
				) 
		);
	...
	public function countCheckinsToPlaceTypesLastWeek($author_id, $network) {
		...
		/*
		 * As we are getting posts from the last week, we might not have any so return a blank string in that instance So we know not to draw this graph on the dashboard.
		 */
	...
	public function getPostsPerHourDataVis($author_id, $network) {
		...
		while ( $i < 24 ) { // Hour 0 through 23
			/*
			 * Data needs to be in format {c:[{v: 'Hour'}, {v: Number of checkins at this hour}]} e.g. [{c:[{v: '11'}, {v: 5}]}
			 */
		...
		$metadata = array (
				...
				array (
						'type' => 'number',
						'label' => 'Checkins Last Week' 
				),
				array (
						'type' => 'number',
						'label' => 'Checkins All Time' 
				) 
		);
	 ...
	 * @return array Posts with link object set
	 */
	public function getAllPostsByHashtagId($hashtag_id, $network, $count, $order_by = "pub_date", $direction = "DESC", $page = 1, $is_public = false) {
		$direction = $direction == "DESC" ? "DESC" : "ASC";
		$start_on_record = ($page - 1) * $count;
		$order_by = $this->sanitizeOrderBy ( $order_by );
