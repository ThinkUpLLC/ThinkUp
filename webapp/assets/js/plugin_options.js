...
var PluginOptions = function() {
        ...
        $(document).ready(function() {
            $("#plugin_option_form").submit(function(event) {
                if (plugin_options.DEBUG) {
                    console.debug("option form submitted");
                }
    ...
    this.submitForm = function() {
        if (plugin_options.submitting) {
            return;
        }
        if (plugin_options.DEBUG) {
            console.debug("submit_form function called...");
        }
        submit_status = true;
        // note plugin_id , option_elements, option_required_message,
        // option_etc,
        // defined in _plugin_options.tpl, and populated in
        ...
        for ( var option_name in option_elements) {
            ...
            status = true;
            if (plugin_options.DEBUG) {
                console.debug('%s type %s id %s', option_name, option.type,
                        option.id);
            }
            ...
            if (plugin_options.DEBUG) {
                console.debug('Submitted stats = %s - %s', submit_status,
                        option_name);
            }
        }
        if (!submit_status) {
            if (plugin_options.DEBUG) {
                console.debug('not submitted');
            }
            ...
            $.ajax( {
                        ...
                        error : function(data) {
                            ...
                            $('#plugin_option_server_error_message')
                                    .html(
                        ...
                        success : function(data) {
                            ...
                            if (data.status == 'failed') {
                                ...
                                // set our ids for new items, so we can update
                                // again in thesame form
                                for (key in data.results.inserted) {
                                    option_elements[key]['id'] = data.results.inserted[key];
                                }
                                setTimeout(
                                        function() {
                                            $('#plugin_options_success')
                                                    .fadeOut(
                                                            500,
                                                            function() {
                                                                plugin_options.submitting = false;
    ...
    this.setRequiredMessage = function(option) {
        ...
        if (option_not_required[option['name']] && !option.regex_fail) {
            if (plugin_options.DEBUG) {
                console.debug('Value not required for %s', option.name);
            }
            return true;
        }
        ...
        if (plugin_options.DEBUG) {
            console.debug("set missing element message %", message);
        }
    ...
    this.processSelectElement = function(option) {
        ...
        if (plugin_options.DEBUG) {
            console.debug("%s form value = %s", option.name, value);
        }
    ...
    this.processTextElement = function(option) {
        ...
        if (value && value != '') {
            ...
            if (plugin_options.DEBUG) {
                console.debug('No value for %s', option.name);
            }
            return false;
        }
    }
