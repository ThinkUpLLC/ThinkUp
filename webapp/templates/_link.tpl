		...
		<li class="individual-tweet">
			<div class="person-info">
				...
				{if $l->container_tweet->mention_count_cache > 0}<h4 class="reply-count"><a href="{$cfg->site_root_path}post/?t={$t->post_id}">{$l->container_tweet->mention_count_cache} repl{if $l->container_tweet->mention_count_cache eq 1}y{else}ies{/if}</a></h4>{/if} 
			</div>
			...
			<div class="tweet-body">
				...
				<h3><a href="http://twitter.com/{$l->container_tweet->author_username}/post/{$l->container_tweet->post_id}">{$l->container_tweet->adj_pub_date|relative_datetime}</a><h3>
