...
class BitlyAPIAccessor {
    ...
    public function BitlyAPIAccessor($bitly_api_key, $bitly_login) {
    ...
    public function getBitlyLinkData($u) {
        if ($this->api_key != '' && $this->login != '') {
            $this->logger->logInfo("Bitly API key set", __METHOD__.','.__LINE__);
            $params = array('shortUrl'=>$u, 'login'=>$this->login, 'apiKey'=>$this->api_key, 'format'=>$this->format);
            ...
            if ($bit_link['error'] == '') {
                ...
                return array("expanded_url"=>'', "title"=>'', "clicks"=>'', "error"=>'No response from Bitly API');
            }
        } else {
    ...
    public function bitlyAPISwitcher($method, $encoded_params) {
        $api_call = $this->api_url.$method.implode('&', $encoded_params);
            ...
            if ($resp != false) {
                ...
                $bit_link['status_txt'] = 'No response from Bit.ly API';
                return array("expanded_url"=>'', "title"=>'', "clicks"=>'', "error"=>'No response from Bitly API');
            }
