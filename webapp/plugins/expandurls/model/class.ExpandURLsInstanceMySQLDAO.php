 ...
 */
class ExpandURLsInstanceMySQLDAO extends InstanceMySQLDAO implements InstanceDAO {

    public function __construct() {
        parent::__construct();
        $this->setObjectName('ExpandURLsInstance');
        $this->setMetaTableName('instances_expandedurls');
    }
    ...
    private function insertMetaData($instance_object) {
        ...
        $vars = array(
            ...
            ':cursor'             => $instance_object->cursor,
            ':finished_checking'  => $instance_object->finished_checking,
        );
        if (!$lfi){
            unset ($vars[':cursor']);;
        }
    ...
    private function updateMetaData($instance_object) {
        $lfi = ($instance_object->id != "" ? true : false);
        $q  = "UPDATE ".$this->getMetaTableName()." SET ";
        if ($lfi){
            $q .= "id = :id, ";
        }
        $q .= "cursor = :cursor, ";
        $q .= "finished_checking = :finished_checking ";
        $q .= "WHERE id=:id;";
	...
	public function updateCursor($instance_object) {}
}
