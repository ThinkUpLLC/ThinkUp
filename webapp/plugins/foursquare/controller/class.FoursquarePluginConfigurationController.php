...
/**
 *
 * webapp/plugins/foursquare/controller/class.FoursquarePluginConfigurationController.php
 *
 * LICENSE:
 *
...
class FoursquarePluginConfigurationController extends PluginConfigurationController {
    ...
    // This method generates the account index page where the user can configure the plugin
    ...
    protected function setUpFoursquareInteractions(array $options) {
        ...
        if (isset($_GET['code'])) {
            ...
            if (isset($tokens->error)) {
                ...
                $this->addErrorMessage("Oops! Something went wrong while obtaining OAuth tokens.<br>foursquare says \"".
                $tokens->error.".\" Please double-check your settings and try again.", 'authorization');
            } else {
                // If we got some OAuth tokens back, check there valid
                $foursquare_api_accessor = new FoursquareAPIAccessor();
                // Make a query for the users details on foursquare
                $foursquare_user = $foursquare_api_accessor->apiRequest('', $tokens->access_token);
                // If foursquare returned an error after that request
                if (isset($foursquare_user->error)) {
                        $this->addErrorMessage("Oops! Something went wrong querying the foursquare API.
                        <br>foursquare says \"".$foursquare_user->error->code.": ".$foursquare_user->error->message.
