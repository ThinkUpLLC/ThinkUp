        ...
        if ($network == 'twitter' && $this->shouldGenerateMonthlyInsight($this->slug, $instance, 'today', false, 8)) {
            ...
            if ($percentile >= 50 && $my_count > $median) {
                ...
                $insight->text = $this->getVariableCopy(array(
                    "%username has more followers than %percentile% of the %total people %username follows. "
                        . "That means %percentoftotal of %username's friends would reach a bigger audience "
                        . "if %username retweeted them.",
                    "%username has more followers than %percentile% of the typical user %username follows. "
                        . "That means %percentoftotal of %username's friends would reach a bigger audience "
                        . "if %username retweeted them."
                ), array(
                    'total' => $follow_count,
                    'percentoftotal' => $number_under,
