<?php
/*
 Plugin Name: Reply Rate
 Description: What is the rate at which your posts generate responses every week.
 When: Wednesdays
 */
...
class ReplyRateInsight extends InsightPluginParent implements InsightPlugin {
    public function generateInsight(Instance $instance, $last_week_of_posts, $number_days) {
        ...
        if ((date('w') == 3 || $in_test_mode) && count($last_week_of_posts)) {
            ...
            if ($response_factor['value']) {
                ...
                if (isset($last_wed_insight_baseline)) {
                    ...
                    != self::getSyntacticTimeDifference($time_per_response)) {
                        if (isset($last_wed_time_per_response) && $last_wed_time_per_response < $time_per_response) {
                            $insight_text .= ", slower than the previous week's average of one ".$response_type." every "
                            .self::getSyntacticTimeDifference($last_wed_time_per_response);
                        } elseif (isset($last_wed_time_per_response) && $last_wed_time_per_response > $time_per_response) {
                            $insight_text .= ", faster than the previous week's average of one ".$response_type." every "
                            .self::getSyntacticTimeDifference($last_wed_time_per_response);
                        }
                    }
                }
                ...
                $this->insight_dao->insertInsight("reply_rate", $instance->id, $this->insight_date, "Reply rate:",
...
$insights_plugin_registrar->registerInsightPlugin('ReplyRateInsight');
