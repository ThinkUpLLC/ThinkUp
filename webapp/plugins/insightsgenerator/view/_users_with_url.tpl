...
<li class="list-item">
    {include file=$tpl_path|cat:"_user_with_url.tpl" user=$u user_text=$user_text}
</li>
{/foreach}
</ul>

{if $users|@count > 2}<button class="btn btn-default btn-block btn-see-all" data-text="Actually, please hide them">
<span class="btn-text">See all {$users|@count} people</span> <i class="fa fa-chevron-down icon"></i></button>{/if}
