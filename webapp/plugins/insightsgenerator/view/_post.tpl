...
<blockquote class="tweet{if $hide_avatar} tweet-without-photo{/if}">
  {if $post->network eq 'twitter'}<a href="https://twitter.com/intent/user?user_id={$post->author_user_id}" title="{$post->author_username}">{/if}<img src="{$post->author_avatar|use_https}" alt="{$post->author_username}" width="60" height="60" class="img-circle pull-left tweet-photo">{if $post->network eq 'twitter'}</a>{/if}
