...
class TestOfListMembershipInsight extends ThinkUpUnitTestCase {
    ...
    private function buildData($total_lists, $build_history=false, $history_ceiling=50) {
        ...
        if ($build_history) {
            ...
            'network'=>'twitter', 'date'=>date('Y-m-d'), type =>'group_membership_count','count'=>$history_ceiling));

            $builders[] = FixtureBuilder::build('count_history', array('network_user_id'=>'13',
            'network'=>'twitter', 'date'=>date ('Y-m-d', strtotime('-1 day')), 'type' =>'group_membership_count',
            'count'=>($history_ceiling-3)));

            $builders[] = FixtureBuilder::build('count_history', array('network_user_id'=>'13',
            'network'=>'twitter', 'date'=>date ('Y-m-d', strtotime('-2 day')), 'type' =>'group_membership_count',
