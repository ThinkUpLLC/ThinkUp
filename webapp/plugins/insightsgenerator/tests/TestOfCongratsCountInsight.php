...
class TestOfCongratsCountInsight extends ThinkUpInsightUnitTestCase {
    public function setUp(){
        ...
        $instance->network_username = 'congratter';
    ...
    public function testNoCongrats() {
        ...
        $post_builders[] = FixtureBuilder::build('posts', array('pub_date' => date('Y-m-d', strtotime('-1 day')),
            ...
            'post_text' => "Well, I'm no going to say that thing people say about good things."));
    ...
    public function testOneCongrat() {
        ...
        $post_builders = array();
        $post_builders[] = FixtureBuilder::build('posts', array('pub_date' => date('Y-m-d', strtotime('-1 day')),
            'in_reply_to_user_id' => 1234, 'in_reply_to_post_id' => 1,
            'author_username'=> $this->instance->network_username, 'network' => $this->instance->network,
            'post_text' => "Congrats!"));
        $insight_plugin = new CongratsCountInsight();
        ...
        $this->assertEqual($result->headline, "@congratter's friend had some great news!");
        $this->assertEqual($result->text, '@congratter congratulated 1 follower in the past month for this tweet.');
    ...
    public function testMultipleCongratsOfDifferentTypesV1() {
        ...
        $post_builders[] = FixtureBuilder::build('posts', array(
            ...
            'author_username'=> 'congratter', 'network' => 'twitter',
        ...
        $result = $insight_dao->getInsight($insight_plugin->getSlug(), $this->instance->id, $today);
        $this->assertNotNull($result);
        $this->assertEqual($result->headline, "@congratter's friends had some great news!");
        $this->assertEqual($result->text,
            "@congratter congratulated 2 followers in the past month for these tweets.");
    ...
    public function testMultipleCongratsOfDifferentTypesV2() {
        ...
        $result = $insight_dao->getInsight($insight_plugin->getSlug(), $this->instance->id, $today);
        $this->assertNotNull($result);
        $this->assertEqual($result->headline, "Twitter is the place to announce good news!");
        $this->assertEqual($result->text,
            "Here are the tweets that inspired @congratter to congratulate friends online this month.");
        $data = unserialize($result->related_data);
        $this->assertEqual(count($data['posts']), 2);
        $this->assertEqual("I am getting married!", $data['posts'][0]->post_text);
        $this->assertEqual("I love cheese!", $data['posts'][1]->post_text);
    ...
    public function testMultipleCongratsOfDifferentTypesV3() {
        ...
        $result = $insight_dao->getInsight($insight_plugin->getSlug(), $this->instance->id, $today);
        $this->assertNotNull($result);
        $this->assertEqual($result->headline, "Congrats on the Congrats, @congratter!");
        $this->assertEqual($result->text,
            "3 tweets inspired @congratter to congratulate a follower on Twitter this past month.");
        $data = unserialize($result->related_data);
        $this->assertEqual(count($data['posts']), 2);
        $this->assertEqual("I am getting married!", $data['posts'][0]->post_text);
        $this->assertEqual("I love cheese!", $data['posts'][1]->post_text);
    ...
    public function testSingularV1() {
        ...
        $this->assertEqual($result->headline, "@congratter's friend had some great news!");
        $this->assertEqual($result->text,
            "@congratter congratulated 1 follower in the past month for this tweet.");
        $data = unserialize($result->related_data);
    ...
    public function testSingularV2() {
        ...
        $this->assertEqual($result->headline, "Twitter is the place to announce good news!");
        $this->assertEqual($result->text,
            "Here is the tweet that inspired @congratter to congratulate friends online this month.");
        $data = unserialize($result->related_data);
        $this->assertEqual(count($data['posts']), 1);
        $this->assertEqual("I am getting married!", $data['posts'][0]->post_text);
        $this->assertEqual("friend1", $data['posts'][0]->author_username);
    ...
    public function testSingularV3() {
        ...
        $this->assertEqual($result->headline, "Congrats on the Congrats, @congratter!");
        $this->assertEqual($result->text,
            "1 tweet inspired @congratter to congratulate a follower on Twitter this past month.");
        $data = unserialize($result->related_data);
        $this->assertEqual(count($data['posts']), 1);
        $this->assertEqual("I am getting married!", $data['posts'][0]->post_text);
        $this->assertEqual("friend1", $data['posts'][0]->author_username);
