...
/**
 *
 * webapp/plugins/insightsgenerator/tests/TestOfBestieInsight.php
 *
 * LICENSE:
 *
...
require_once THINKUP_ROOT_PATH. 'webapp/plugins/insightsgenerator/insights/bestie.php';

class TestOfBestieInsight extends ThinkUpInsightUnitTestCase {
    ...
    public function testConstructor() {
        ...
        $this->assertIsA($insight_plugin, 'BestieInsight' );
    }

    public function testTwitterCompleteDataNoStreak() {
        $this->instance = new Instance();
        $this->instance->id = 1;
        ...
        $this->dumpRenderedInsight($result, $this->instance, "Twitter complete data no streak");
    }

    public function testTwitterCompleteDataStreakMatch() {
        $this->instance = new Instance();
        $this->instance->id = 1;
        $this->instance->network_user_id = '13';
        $this->instance->network_username = 'ev';
        ...
        $builders[] = FixtureBuilder::build('insight_baselines', array('slug'=>'bestie_last_bestie_id', 'value'=>'19',
            'date'=>date(strtotime('-1 month')), 'instance_id'=>1));
        $builders[] = FixtureBuilder::build('insight_baselines', array('slug'=>'bestie_last_bestie_count', 'value'=>'3',
            'date'=>date(strtotime('-1 month')), 'instance_id'=>1));
        ...
        $sql = "SELECT * FROM tu_insight_baselines WHERE slug = 'bestie_last_bestie_count' and date = '".
            date('Y-m-d')."'";
        $stmt = InsightBaselineMySQLDAO::$PDO->query($sql);
        $data = $stmt->fetch(PDO::FETCH_ASSOC);
        $this->assertEqual('4', $data['value']);

        $this->dumpRenderedInsight($result, $this->instance, "Twitter complete data streak exists, matches");
    }

    public function testTwitterCompleteDataStreakNoMatch() {
        $this->instance = new Instance();
        $this->instance->id = 1;
        $this->instance->network_user_id = '13';
        $this->instance->network_username = 'ev';
        ...
        $builders[] = FixtureBuilder::build('insight_baselines', array('slug'=>'bestie_last_bestie_id', 'value'=>'29',
            'date'=>date(strtotime('-1 month')), 'instance_id'=>1));
        $builders[] = FixtureBuilder::build('insight_baselines', array('slug'=>'bestie_last_bestie_count', 'value'=>'3',
            'date'=>date(strtotime('-1 month')), 'instance_id'=>1));
    ...
    public function testFacebookCompleteDataNoStreak() {
        $this->instance = new Instance();
        $this->instance->id = 1;
        ...
        //commented on fbuser1's status updates 5 times, more than anyone else. You two!
    ...
    protected function buildTruckloadOfTwitterData() {
        ...
        'full_name'=>'Link Baiter', 'avatar'=>'https://farm7.staticflickr.com/6146/5976784449_4fe7c02760_q.jpg',
        'is_protected'=>0, 'follower_count'=>70,
        'network'=>'twitter'));
