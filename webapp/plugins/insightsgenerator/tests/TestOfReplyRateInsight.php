...
/**
 *
 * ThinkUp/webapp/plugins/insightsgenerator/tests/TestOfReplyRateInsight.php
 *
 ...
 *
 * Test of ReplyRateInsight
 *
 * Test for the ReplyRateInsight class.
 *
...
require_once THINKUP_ROOT_PATH. 'webapp/plugins/insightsgenerator/insights/replyrate.php';

class TestOfReplyRateInsight extends ThinkUpUnitTestCase {
    ...
    public function testGetSyntacticTimeDifference() {
        ...
        $result_1 = ReplyRateInsight::getSyntacticTimeDifference($delta_1);
        $result_2 = ReplyRateInsight::getSyntacticTimeDifference($delta_2);
        $result_3 = ReplyRateInsight::getSyntacticTimeDifference($delta_3);
        $result_4 = ReplyRateInsight::getSyntacticTimeDifference($delta_4);
    ...
    public function testReplyRateInsightForTwitterNoPriorBaseline() {
        ...
        $time_per_response = ReplyRateInsight::getSyntacticTimeDifference(floor((60 * 60 * 24 * 7) / 25));

        $insight_plugin = new ReplyRateInsight();
        $insight_plugin->generateInsight($instance, $posts, 3);

        // Assert that insight got inserted
        $insight_dao = new InsightMySQLDAO();
        $today = date ('Y-m-d');
        $result = $insight_dao->getInsight('reply_rate', 10, $today);
    ...
    public function testReplyRateInsightForFacebookPriorGreaterBaseline() {
        // Get data ready that insight requires
        $instance = new Instance();
        $instance->id = 10;
        $instance->network_username = 'testeriffic';
        ...
        $time_per_response = ReplyRateInsight::getSyntacticTimeDifference(floor((60 * 60 * 24 * 7) / 19));
        $last_week_time_per_response = ReplyRateInsight::getSyntacticTimeDifference(floor((60 * 60 * 24 * 7) / 27));
    ...
    public function testReplyRateInsightForFoursquarePriorSmallerBaseline() {
        // Get data ready that insight requires
        $instance = new Instance();
        $instance->id = 10;
        $instance->network_username = 'testeriffic';
        ...
        // Calculate time for each new favorite
        $time_per_response = ReplyRateInsight::getSyntacticTimeDifference(floor((60 * 60 * 24 * 7) / 17));
        $last_week_time_per_response = ReplyRateInsight::getSyntacticTimeDifference(floor((60 * 60 * 24 * 7) / 12));
