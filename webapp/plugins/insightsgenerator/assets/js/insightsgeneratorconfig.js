/**
 * 
 * ThinkUp/webapp/plugins/insightsgenerator/assets/js/insightsgeneratorconfig.js
 * 
 * Copyright (c) 2013-2014 Pranjal Goswami
 * 
 * LICENSE:
 * 
 ...
 * 
 * 
 * @author Pranjal Goswami <pranjal.b.goswami[at]gmail[dot]com>
 * @license http://www.gnu.org/licenses/gpl.html
 * @copyright 2013-2014 Pranjal Goswami
 */

var TUInsightsGeneratorSettings = function() {
    ...
    this.init = function() {
        ...
        $(document).ready(function() {
                tu_insightsgenerator_settings.load_settings();
        });

    };

    this.save_settings = function() {
        $('#plugin_settings_error_message_error').hide();
        if (this.DEBUG) {
        ...
        for (key in this.settings_data.insightsgenerator_config_settings) {
            var setting = this.settings_data.insightsgenerator_config_settings[key];
            var id = '#' + key;
        ...
        if (this.DEBUG) {
            console.debug("saving insightsgenerator settings");
            console.dir( {
                save_settings : params
            });
        }
        $('#insightsgenerator-settings-save').hide();
        $('#save_plugin_setting_image').show();
        controller_uri = site_root_path + 'plugins/insightsgenerator/controller/insightsgeneratorconfig.php';
        $
                .ajax( {
                    url : controller_uri,
                    data : params,
                    ...
                    dataType : 'json',
                    error : function(data) {
                        $('#insightsgenerator-settings-save').show();
                        $('#save_plugin_setting_image').hide();
                        $('#plugin_settings_error_message')
                                .html(
                                        'Sorry, but we are unable to process your request at this time.');
                        $('#plugin_settings_error_message_error').show();
                    },
                    success : function(data) {
                        tu_insightsgenerator_settings._save_settings(data);
                    }
                });
    ...
    this._save_settings = function(data) {
        ...
        if (data.status == 'failed') {
            if (data.required) {
                ...
                error_mess += '</ul>';
                $('#plugin_settings_error_message').html(error_mess);
                $('#plugin_settings_error_message_error').show();
            }
        } else {
            $('#plugin_settings_success').show();
            setTimeout(function() {
                $('#plugin_settings_success').fadeOut(1500, function() {
                });
            }, 500);
        }
    };
    this.load_settings = function() {
        if (!tu_insightsgenerator_settings.TAB_LOADED) {
            if (this.DEBUG) {
            ...
            $
                    .ajax( {
                        ...
                        dataType : 'json',
                        error : function(data) {
                            $('#insightsgenerator_setting_loading_div').hide();
                            $('#insightsgenerator_settings_div').show();
                            $('#plugin_settings_error_message')
                                    .html(
                                            'Sorry, but we are unable to process your request at this time.');
                            $('#plugin_settings_error_message_error').show();
                        ...
                        success : function(data) {
                            tu_insightsgenerator_settings._load_settings(data);
                        }
                    });
        } else {
            if (this.DEBUG) {
    ...
    this._load_settings = function(data) {
        ...
        $('#insightsgenerator-settings-form').submit(function() {
            tu_insightsgenerator_settings.save_settings();
        });
        for (key in data.insightsgenerator_config_settings) {
            var setting = data.insightsgenerator_config_settings[key];
            var id = '#' + key;
            if (data.values[key]) {
                if (this.DEBUG) {
...
var tu_insightsgenerator_settings = new TUInsightsGeneratorSettings();
tu_insightsgenerator_settings.init();
