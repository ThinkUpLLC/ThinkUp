<?php 
class TwitterPlugin implements iCrawlerPlugin, iWebappPlugin {
    ...
    public function getChildTabsUnderPosts() {
        ...
        $alltabds = new WebappTabDataset("all_tweets", $pd, "getAllPosts", array($i->network_user_id, 15));
        ...
        $mrttabds = new WebappTabDataset("most_replied_to_tweets", $pd, "getMostRepliedToPosts", array($i->network_user_id, 15));
        ...
        $mstabds = new WebappTabDataset("most_retweeted", $pd, "getMostRetweetedPosts", array($i->network_user_id, 15));
        $mstab->addDataset($mstabds);
        array_push($child_tabs, $mstab);
        
        // Conversations
        $convotab = new WebappTab("tweets-convo", "Conversations", "", $twitter_data_tpl);
        $convotabds = new WebappTabDataset("author_replies", $pd, "getPostsAuthorHasRepliedTo", array($i->network_user_id, 15));
    ...
    public function getChildTabsUnderReplies() {
        ...
        $amtabds1 = new WebappTabDataset("all_tweets", $pd, "getAllPosts", array($i->network_user_id, 15));
        $amtabds2 = new WebappTabDataset("all_mentions", $pd, "getAllMentions", array($i->network_username, 15, $i->network));
        ...
        $artabds = new WebappTabDataset("all_replies", $pd, "getAllReplies", array($i->network_user_id, 15));
        ...
        $omtabds1 = new WebappTabDataset("all_tweets", $pd, "getAllPosts", array($i->network_user_id, 15));
        $omtabds2 = new WebappTabDataset("orphan_replies", $pd, "getOrphanReplies", array($i->network_username, 5, $i->network));
        ...
        $sttabds1 = new WebappTabDataset("standalone_replies", $pd, "getStandaloneReplies", array($i->network_username, 15));
        $sttabds2 = new WebappTabDataset("all_tweets", $pd, "getAllPosts", array($i->network_user_id, 15));
    ...
    public function getChildTabsUnderFriends() {
        ...
        $motabds = new WebappTabDataset('people', $fd, "getMostActiveFollowees", array($i->network_user_id, 15));
        ...
        $latabds = new WebappTabDataset("people", $fd, "getLeastActiveFollowees", array($i->network_user_id, 15));
        $latab->addDataset($latabds);
        array_push($child_tabs, $latab);
        
        //Popular friends
        $poptab = new WebappTab("friends-mostfollowed", 'Popular', '', $twitter_data_tpl);
        $poptabds = new WebappTabDataset("people", $fd, "getMostFollowedFollowees", array($i->network_user_id, 15));
        $poptab->addDataset($poptabds);
        array_push($child_tabs, $poptab);
        
        //Former Friends
        $fftab = new WebappTab("friends-former", "Former", '', $twitter_data_tpl);
        $fftabds = new WebappTabDataset("people", $fd, "getFormerFollowees", array($i->network_user_id, 15));
        $fftab->addDataset($fftabds);
        array_push($child_tabs, $fftab);
        
        //Not Mutual Friends
        $nmtab = new WebappTab("friends-notmutual", "Not Mutual", '', $twitter_data_tpl);
        $nmtabds = new WebappTabDataset("people", $fd, "getFriendsNotFollowingBack", array($i->network_user_id));
    ...
    public function getChildTabsUnderFollowers() {
        ...
        $mftabds = new WebappTabDataset("people", $fd, "getMostFollowedFollowers", array($i->network_user_id, 15));
        ...
        $lltabds = new WebappTabDataset("people", $fd, "getLeastLikelyFollowers", array($i->network_user_id, 15));
        $lltab->addDataset($lltabds);
        array_push($child_tabs, $lltab);
        
        //Former followers
        $fftab = new WebappTab("followers-former", "Former", '', $twitter_data_tpl);
        $fftabds = new WebappTabDataset("people", $fd, "getFormerFollowers", array($i->network_user_id, 15));
        $fftab->addDataset($fftabds);
        array_push($child_tabs, $fftab);
        
        //Earliest
        $eftab = new WebappTab("followers-earliest", "Earliest Joiners", '', $twitter_data_tpl);
        $eftabds = new WebappTabDataset("people", $fd, "getEarliestJoinerFollowers", array($i->network_user_id, 15));
    ...
    public function getChildTabsUnderLinks() {
        ...
        $fltabds = new WebappTabDataset("links", $ld, "getLinksByFriends", array($i->network_user_id));
        ...
        /* $lftab = new WebappTab("links-favorites", 'Links From Favorites', 'Links in posts you favorited');
         $lftabds = new WebappTabDataset("links", $ld, "getLinksByFriends", array($i->network_user_id));
        ...
        $ptabds = new WebappTabDataset("links", $ld, "getPhotosByFriends", array($i->network_user_id));
