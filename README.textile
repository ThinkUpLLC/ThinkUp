...
* Rename <code>thinktank/webapp/config.sample.inc.php</code> to <code>config.inc.php</code> and set the appropriate Application, Database, and Plugin values for your server
...
Then, run the crawler to load your Twitter data, <code>cd</code> to <code>/your-path-to-thinktank/webapp/crawler/</code>, and run:

<code>$ php crawl.php</code>
...
ThinkTank's source code is licensed under the "GNU General Public License":http://github.com/ginatrapani/thinktank/blob/master/GPL-LICENSE.txt, except for the externa libraries listed below.
...
* "Smarty":http://smarty.net (Not included)
